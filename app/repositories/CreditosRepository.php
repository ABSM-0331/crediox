...
class CreditosRepository
{
    ...
    public function obtenerTodos()
    {
        $sql = "SELECT 
                    ...
                    CONCAT(cob.ap_paterno, ' ', cob.ap_materno, ' ', cob.nombres) AS cobratario
                FROM creditos c
                INNER JOIN personas cli ON c.idcliente = cli.idpersona AND cli.activo = 1
                LEFT JOIN personas cob ON c.idcobratario = cob.idpersona AND cob.activo = 1
                WHERE c.idcobratario IS NULL OR cob.idpersona IS NOT NULL
    ...
    public function obtenerCreditosCliente($idCliente)
    {
        $sql = "SELECT c.*, 
                ...
                FROM creditos c
                INNER JOIN personas cl ON c.idcliente = cl.idpersona AND cl.activo = 1
                LEFT JOIN personas cb ON c.idcobratario = cb.idpersona AND cb.activo = 1
                WHERE c.idcliente = ?";
    ...
    public function obtenerCreditosCobratario($idCobratario)
    {
        $sql = "SELECT c.*, 
                ...
                FROM creditos c
                INNER JOIN personas cl ON c.idcliente = cl.idpersona AND cl.activo = 1
                INNER JOIN personas cob ON c.idcobratario = cob.idpersona AND cob.activo = 1
    ...
    public function obtenerTotalCobradoCobratario($idCobratario): float
    {
        $sql = "SELECT COALESCE(SUM(hp.monto_pagado), 0) AS total_cobrado
                ...
                INNER JOIN personas cli ON cli.idpersona = c.idcliente AND cli.activo = 1
                INNER JOIN personas cob ON cob.idpersona = c.idcobratario AND cob.activo = 1
                WHERE c.idcobratario = ?";
    ...
    public function obtenerTotalCobradoHoyGlobal(): float
    {
        $sql = "SELECT COALESCE(SUM(monto_pagado), 0) AS total_cobrado
                                ...
                                INNER JOIN creditos c ON c.idcredito = hp.idcredito
                                INNER JOIN personas cli ON cli.idpersona = c.idcliente AND cli.activo = 1
                                LEFT JOIN personas cob ON cob.idpersona = c.idcobratario AND cob.activo = 1
                                WHERE hp.fecha_pago = CURDATE()
                                    AND (c.idcobratario IS NULL OR cob.idpersona IS NOT NULL)";
    ...
    public function obtenerPendienteCobroHoyGlobal(): float
    {
        $sql = "SELECT COALESCE(SUM(monto_programado), 0) AS pendiente_hoy
                                ...
                                INNER JOIN personas cli ON cli.idpersona = c.idcliente AND cli.activo = 1
                                LEFT JOIN personas cob ON cob.idpersona = c.idcobratario AND cob.activo = 1
                                WHERE DATE(pc.fecha_programada) = CURDATE()
                                    AND pc.estado = 'pendiente'
                                    AND (c.idcobratario IS NULL OR cob.idpersona IS NOT NULL)";
    ...
    public function obtenerEstadisticasDashboard(): array
    {
        try {
            ...
            $sqlClientes = "SELECT COUNT(*) as total FROM personas WHERE idrol = 2 AND activo = 1";
            ...
            $sqlCobratarios = "SELECT COUNT(*) as total FROM personas WHERE idrol = 3 AND activo = 1";
            ...
            $sqlCobrosPendientes = "SELECT COUNT(*) as total 
                                                                    ...
                                                                    INNER JOIN personas cli ON cli.idpersona = c.idcliente AND cli.activo = 1
                                                                    LEFT JOIN personas cob ON cob.idpersona = c.idcobratario AND cob.activo = 1
                                                                    WHERE DATE(pc.fecha_programada) = :hoy 
                                                                        AND pc.estado = 'pendiente'
                                                                        AND (c.idcobratario IS NULL OR cob.idpersona IS NOT NULL)";

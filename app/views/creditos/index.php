...
$creditosActivos = array_values(array_filter($creditos, static function (array $credito): bool {
    return strtolower((string)($credito['estado'] ?? '')) === 'activo';
}));

$creditosCompletados = array_values(array_filter($creditos, static function (array $credito): bool {
    return strtolower((string)($credito['estado'] ?? '')) === 'completado';
}));

$creditosCerrados = array_values(array_filter($creditos, static function (array $credito): bool {
    $estado = strtolower((string)($credito['estado'] ?? ''));
    return $estado !== 'activo' && $estado !== 'completado';
}));

$creditosNoActivos = array_merge($creditosCompletados, $creditosCerrados);
...
<section id="creditos" class="content-section">
    ...
    <div id="tipos-credito" style="margin-bottom: 30px;">
        <div style="background: var(--bg-secondary); padding: 18px; border-radius: 8px; margin-bottom: 15px;">
            ...
            <form method="POST" action="/panel/public/creditos/tipos/guardar" class="tipos-credito-create-form">
            ...
            <?php else: ?>
                <div class="tipos-credito-lista">
                    ...
                    <?php foreach ($tiposCredito as $itemTipo): ?>
                        <form method="POST" action="/panel/public/creditos/tipos/actualizar" class="tipos-credito-row-form">
                            ...
                            <div class="tipo-acciones">
                                ...
                                <button type="submit" formaction="/panel/public/creditos/tipos/eliminar" formmethod="POST" onclick="return confirm('¿Eliminar este tipo de crédito?');" class="btn-tipo-eliminar" <?= ((int)($itemTipo['es_flexible'] ?? 0) === 1 ? 'disabled' : '') ?>>Eliminar</button>
                            </div>
                        </form>
                    <?php endforeach; ?>
                </div>
    ...
    <div id="tablaCreditos" style="margin-bottom: 30px;">
        <div style="background: var(--bg-secondary); padding: 15px; border-radius: 8px; margin-bottom: 15px;">
            <h3 style="margin: 0 0 15px 0; color: var(--text-primary); font-size: 16px;">Créditos Registrados</h3>
            ...
            <?php else: ?>
                ...
                <div style="display: grid; gap: 12px;">
                    <details id="panelCreditosAdminActivos" open style="background: var(--bg-primary); border: 1px solid var(--border-color); border-radius: 8px; overflow: hidden;">
                        <summary style="cursor: pointer; list-style: none; padding: 12px 14px; background: var(--bg-tertiary); color: var(--text-primary); font-weight: 700; font-size: 14px;">
                            Créditos Activos (<?= count($creditosActivos) ?>)
                        </summary>
                        <?php if (empty($creditosActivos)): ?>
                            <div style="padding: 14px; color: var(--text-muted); font-size: 13px;">No hay créditos activos.</div>
                        <?php else: ?>
                            <div style="overflow-x: auto; border-top: 1px solid var(--border-color);">
                                <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
                                    ...
                                    <tbody class="tbody-creditos-admin">
                                        <?php foreach ($creditosActivos as $credito): ?>
                                            <tr data-search-row-admin style="border-bottom: 1px solid var(--border-color);">
                                                ...
                                                <td style="padding: 12px; text-align: center;">
                                                    <span style="display: inline-block; padding: 4px 8px; border-radius: 4px; font-size: 12px; font-weight: 500; background: rgba(34, 197, 94, 0.1); color: rgb(34, 197, 94);">
                    ...
                    <details id="panelCreditosAdminCompletados" style="background: var(--bg-primary); border: 1px solid var(--border-color); border-radius: 8px; overflow: hidden;">
                        <summary style="cursor: pointer; list-style: none; padding: 12px 14px; background: var(--bg-tertiary); color: var(--text-primary); font-weight: 700; font-size: 14px;">
                            Créditos Completados y Cerrados (<?= count($creditosNoActivos) ?>)
                        </summary>
                        <?php if (empty($creditosNoActivos)): ?>
                            <div style="padding: 14px; color: var(--text-muted); font-size: 13px;">No hay créditos completados o cerrados.</div>
                        <?php else: ?>
                            <div style="overflow-x: auto; border-top: 1px solid var(--border-color);">
                                <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
                                    <thead style="background: var(--bg-tertiary); position: sticky; top: 0;">
                                    ...
                                    <tbody class="tbody-creditos-admin">
                                        <?php foreach ($creditosNoActivos as $credito): ?>
                                            <tr data-search-row-admin style="border-bottom: 1px solid var(--border-color);">
                                                <td style="padding: 12px; text-align: center; color: var(--text-primary); font-weight: 500;">#<?= htmlspecialchars($credito['idcredito']) ?></td>
                                                <td style="padding: 12px; text-align: center; color: var(--text-primary);"><?= htmlspecialchars($credito['cliente']) ?></td>
                                                <td style="padding: 12px; text-align: center; color: var(--text-primary);"><?= htmlspecialchars($credito['cobratario']) ?></td>
    ...
    <div id="simuladorCreditos" style="display: none;">
        ...
        <form class="form-card" method="POST" action="/panel/public/creditos">
        ...
        <div id="resultadoSimulacion" style="margin-top: 40px;<?= $simular ? '' : ' display: none;' ?>">
            <?php if ($simular && $clienteSeleccionado): ?>
                ...
                ?>
                    ...
                    <form id="guardarCreditoForm" method="POST" action="/panel/public/creditos/guardar" style="display: none;">
...
<script>
    ...
    function inicializarBuscadorCreditosAdmin() {
        ...
        const filas = document.querySelectorAll('tr[data-search-row-admin]');
        const sinResultados = document.getElementById('sinResultadosCreditosAdmin');
        const panelActivos = document.getElementById('panelCreditosAdminActivos');
        const panelCompletados = document.getElementById('panelCreditosAdminCompletados');
        ...
        buscador.addEventListener('input', function() {
            ...
            if (termino !== '') {
                if (panelActivos) {
                    panelActivos.open = true;
                }
                if (panelCompletados) {
                    panelCompletados.open = true;
                }
            }
    ...
    function verDetalleCredito(idCredito) {
        ...
        fetch(`/panel/public/creditos/obtener?id=${idCredito}`)
            .then(response => {
                if (!response.ok) {

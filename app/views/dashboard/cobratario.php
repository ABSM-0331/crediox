        ...
        <div style="background: var(--bg-secondary); padding: 15px; border-radius: 8px; margin-bottom: 15px;">
            ...
            <?php else: ?>
                ...
                <div style="display: grid; gap: 12px;">
                    <details id="panelCreditosActivos" open style="background: var(--bg-primary); border: 1px solid var(--border-color); border-radius: 8px; overflow: hidden;">
                        ...
                        <?php else: ?>
                            <div style="overflow-x: auto; border-top: 1px solid var(--border-color);">
                                <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
                                    ...
                                    <tbody class="tbody-creditos-cobratario">
                                        <?php foreach ($creditosActivos as $credito): ?>
                                            <tr data-search-row style="border-bottom: 1px solid var(--border-color);">
                    ...
                    <details id="panelCreditosCompletados" style="background: var(--bg-primary); border: 1px solid var(--border-color); border-radius: 8px; overflow: hidden;">
                        <summary style="cursor: pointer; list-style: none; padding: 12px 14px; background: var(--bg-tertiary); color: var(--text-primary); font-weight: 700; font-size: 14px;">
                                        ...
                                        <?php foreach ($creditosNoActivos as $credito): ?>
                                            <tr data-search-row style="border-bottom: 1px solid var(--border-color);">
                                                ...
                                                <td style="padding: 12px; text-align: center;">
                                                    ...
                                                    <span style="color: var(--text-muted); font-size: 12px;">Sin cobro</span>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                                    </tbody>
...
<script>
    ...
    function inicializarBuscadorCreditosCobratario() {
        ...
        const filas = document.querySelectorAll('tr[data-search-row]');
        const sinResultados = document.getElementById('sinResultadosCreditosCobratario');
        const panelActivos = document.getElementById('panelCreditosActivos');
        const panelCompletados = document.getElementById('panelCreditosCompletados');
                ...
                .then((data) => {
                    ...
                    Swal.fire({
                        ...
                        if (historialIds.length > 0) {
                            const urlRecibo = `/panel/public/creditos/recibo?historial=${historialIds.join(',')}&idcredito=${idCreditoActual}`;
                            window.open(urlRecibo, 'recibo_cobro', 'width=900,height=1000,scrollbars=yes');
                        } else if (recibos.length > 0 && recibos[0].idhistorial) {
                            const urlRecibo = `/panel/public/creditos/recibo?historial=${recibos[0].idhistorial}&idcredito=${idCreditoActual}`;
                            window.open(urlRecibo, 'recibo_cobro', 'width=900,height=1000,scrollbars=yes');
                        } else if (idPago && idCredito) {
                            const urlRecibo = `/panel/public/creditos/recibo?idpago=${idPago}&idcredito=${idCredito}`;
                            window.open(urlRecibo, 'recibo_cobro', 'width=900,height=1000,scrollbars=yes');
                        }
    ...
    function recuperarTicketPago(idCredito, idPago) {
        const ticketUrl = `/panel/public/creditos/ver-ticket?idpago=${idPago}&idcredito=${idCredito}`;

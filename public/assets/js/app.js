...
function cargarEstados() {
    fetch("/panel/public/api/estados")
            ...
            if (result.success) {
                ...
                if (selectEstadoCliente) {
                    ...
                    const estadoSeleccionado =
                        selectEstadoCliente.dataset.selectedEstado || "";
                    const municipioSeleccionado =
                        document.getElementById("idmunicipio")?.dataset
                            .selectedMunicipio || "";

                    if (estadoSeleccionado) {
                        selectEstadoCliente.value = estadoSeleccionado;
                        const selectMunicipioCliente =
                            document.getElementById("idmunicipio");
                        if (selectMunicipioCliente) {
                            cargarMunicipios(
                                estadoSeleccionado,
                                selectMunicipioCliente,
                ...
                if (selectEstadoCobratario) {
                    ...
                    const estadoCobSeleccionado =
                        selectEstadoCobratario.dataset.selectedEstado || "";
                    const municipioCobSeleccionado =
                        document.getElementById("cob_idmunicipio")?.dataset
                            .selectedMunicipio || "";

                    if (estadoCobSeleccionado) {
                        selectEstadoCobratario.value = estadoCobSeleccionado;
                        const selectMunicipioCobratario =
                            document.getElementById("cob_idmunicipio");
                        if (selectMunicipioCobratario) {
                            cargarMunicipios(
                                estadoCobSeleccionado,
                                selectMunicipioCobratario,
                                municipioCobSeleccionado,
                            );
                        }
                    }
                }
...
function cargarMunicipios(idestado, selectElement, selectedMunicipio = "") {
    // Limpiar y deshabilitar mientras carga
    selectElement.innerHTML =
        '<option value="" disabled selected>Cargando...</option>';
    selectElement.disabled = true;

    fetch(`/panel/public/api/municipios?idestado=${idestado}`)
        .then((response) => response.json())
        .then((result) => {
            if (result.success) {
                selectElement.innerHTML =
                ...
                if (selectedMunicipio) {
                    selectElement.value = selectedMunicipio;
                }
            } else {
                selectElement.innerHTML =
...
document.addEventListener("DOMContentLoaded", function () {
    document.querySelectorAll("[data-delete-client]").forEach((form) => {
        form.addEventListener("submit", function (event) {
            event.preventDefault();

            const clientName = this.dataset.clientName || "este cliente";
            ...
            Swal.fire({
                ...
                text: `¿Deseas eliminar a ${clientName}?`,
